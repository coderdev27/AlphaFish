...
const openAiApiKey : any =  config.get('openAiApiKey')
...
.action(async(apiKey : string,secretKey : string)=>{
    try {
...
.action(async(quantity : number,coinName : string)=>{
    try {
        let formatCoinName : string = '';
...
.action(async(coinName : string,price : number,quantity : number)=>{
...
.action(async(coinName : string,condition : string,fundingRate : number,side : 'buy' | 'sell',quantity : number)=>{
    ...
    setInterval(async()=>{
        ...
        const response : any = await (await fetch(`https://ftx.com/api/funding_rates?start_time=${seconds-16000}&end_time=${seconds}&future=${formatCoinName.toUpperCase()}`)).json()
...
.action(async(time : string,side : "buy" | "sell",coinName : string, quantity : number)=>{
    try {
        let formatCoinName : string = '';
        if(coinName.includes('perp')){
            formatCoinName = `${coinName.replace("perp","-PERP")}`;
...
.action(async(twitterHandle : string,side : "buy" | "sell",quantity : number)=>{
    try {
        

        //     console.log(chalk.redBright(`${twitterHandle} hasn't tweeted yet.`));
        let inverseSide : 'buy' | 'sell' = side;
        if(side === 'sell'){
            inverseSide = 'buy'
        }
